...
public static class TreeIconProvider
{
    ...
    private static Bitmap CreateServerIcon()
    {
        ...
        using (Pen pen = new(frameColor, 1f))
        {
        ...
        // Vents
        using (Pen linePen = new(Color.FromArgb(120, 130, 140), 1))
        {
    ...
    private static Bitmap CreateDatabaseIcon(Color color, bool isActive)
    {
        Bitmap bmp = new(16, 16);
        using Graphics g = Graphics.FromImage(bmp);
        g.Clear(Color.Transparent);
        ...
        // Top lid
        using (LinearGradientBrush lidBrush = new(
        ...
        using (Pen outlinePen = new(outlineColor, 1f))
        {
        ...
        // Side borders
        using (Pen borderPen = new(darkColor, 1f))
        {
    ...
    private static Bitmap CreateFolderIcon()
    {
            ...
            new Rectangle(2, 2, 12, 12), lightColor, baseColor, LinearGradientMode.ForwardDiagonal))
        {
            ...
            using Pen borderPen = new(shadowColor, 1f);
            g.DrawPath(borderPen, path);
        }

        using (LinearGradientBrush flapBrush = new(
    ...
    private static Bitmap CreateTableIcon()
    {
        ...
        // Grid border
        using (Pen borderPen = new(Color.FromArgb(100, 110, 120), 1f))
        {
